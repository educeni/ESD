...
int main()
{
	...
	pessoas = malloc(MAX_PESSOAS * sizeof(Pessoa*));
	if (pessoas == NULL)
	{
		printf("Erro ao alocar memoria para o vetor de ponteiros.\n");
		return 1;
	}
	...
	while (escolha != 0)
	{
		...
		case 1:
			...
			if (quant + qtdCadastro > MAX_PESSOAS)
			{
				printf("O numero excede o limite maximo de %d cadastros.\n", MAX_PESSOAS);
			}
			else
			{
...
int cadastro(int qtdCadastro, Pessoa **pessoas, int *quant)
{
	...
	{
		pessoas[*quant] = malloc(sizeof(Pessoa));
		if (pessoas[*quant] == NULL)
		{
