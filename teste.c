...
void main()
{
	...
	int escolha, qtdCadastro, quant = 0, nomeachado; // Variáveis da função cadastro
	...
	while (escolha != 0)
	{
		...
		case 4:
			...
			if((-1!=verificaNome)&&(-1!=verificaNome)) {
				...
				printf("Distancia euclidiana: %.2f", result);
			}
			else {
...
float Pessoamaissemelhante(float notas[][QTD_PREF],int verificaNome,char nomes[MAX_PESSOAS][50], int col, int *nomeachado) {
	...
	for(i=0; i<col; i++) {
		result = calculaDistancia(notas, verificaNome, i, col);
		if(verificaNome != i) {
...
void afinidade(char nomes[MAX_PESSOAS][50], float notas[][QTD_PREF], char nomepesquisado[], int qtdCadastro)
{
	int indices[MAX_PESSOAS], i;
	float distancias[MAX_PESSOAS], distancia;

	printf("========================================");
	printf("PERFIS MAIS PROXIMOS DE %s", nomepesquisado);
	printf("========================================\n\n");

	//Guardando a distancia no vetor
	for(i=0;i<qtdCadastro;i++)
	{
